    ...
    >
      ...
      <div className="">
        <div className="absolute right-0 top-0 -mr-4  mt-1 hover:border-gray-800 hover:rounded-md border-gray-100 border bg-white ">
          {isOpen ? (
            <ChevronLeft className="w-6 h-6" onClick={toggleSidebar} />
          ) : (
        ...
        {isOpen ? (
          <div className="  md:min-w-[200px] min-w-[120px] m-2">
            ...
            <span className='text-[8px]'>ME</span>
              ...
              <div>
                <p className="md:text-[12px] text-gray-900  font-bold space-x-3">
                  {userInfo.first_name} {userInfo.last_name}
                </p>
                <p className="md:text-[12px] text-[7px] text-natural-500">
                  {userInfo.email}
                </p>
              </div>
            </div>
                  ...
                  {menuList.map((menuGroup, key) => (
                    <CommandGroup key={key} heading={menuGroup.group}>
                      {menuGroup.items.map((menuItem, itemKey) => (
                        <CommandItem
                          key={itemKey}
                          className="flex gap-2 cursor-pointer  hover:bg-sky-200 rounded-md active:bg-sky-700"
                        >
                  ...
                  {menuList.map((menuGroup, key) => (
                    <CommandGroup key={key} heading={menuGroup.group}>
                      {menuGroup.items.map((menuItem, itemKey) => (
                        <CommandItem
                          key={itemKey}
                          className="flex gap-2 cursor-pointer mx-auto py-2 justify-center hover:bg-sky-200 rounded-md active:bg-sky-700"
                        >
                          <Link href={menuItem.link}>{menuItem.icon}</Link>
                        </CommandItem>
                      ))}
                      {/* <CommandItem className="absolute bottom-5 flex flex-col border-t ">
                  ...
                  {settings.map((menuGroup, key) => (
                    <CommandGroup key={key}  className="absolute bottom-1">
                      {menuGroup.items.map((menuItem, itemKey) => (
                        <CommandItem
                          key={itemKey}
                          className="flex gap-2  justify-center items-center cursor-pointer  hover:bg-sky-200 rounded-md active:bg-sky-700"
                        >
                          <Link href={menuItem.link}
                             passHref
                             className="px-2"
